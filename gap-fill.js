...
async function getBtcUsdSeries() {
  const url = `https://pro-api.coingecko.com/api/v3/coins/bitcoin/market_chart/range?vs_currency=usd&from=${GAP_START_UNIX}&to=${NOW_UNIX}`;
  const res = await fetchWithTimeout(url, { headers: { 'x-cg-pro-api-key': process.env.COINGECKO_API_KEY } });
...
async function fillCoinGeckoDirect(currencyCode, btcUsdByDate) {
  ...
  const url = `https://pro-api.coingecko.com/api/v3/coins/bitcoin/market_chart/range?vs_currency=${currencyCode}&from=${GAP_START_UNIX}&to=${NOW_UNIX}`;
  const res = await fetchWithTimeout(url, { headers: { 'x-cg-pro-api-key': process.env.COINGECKO_API_KEY } });
...
async function run() {
  ...
  for (const code of COINGECKO_CURRENCIES) {
    ...
    await sleep(500); // free Demo tier: pace well under 100 calls/min
  }
